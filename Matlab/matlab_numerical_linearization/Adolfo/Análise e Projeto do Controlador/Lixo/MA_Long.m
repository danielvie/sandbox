...
function [Pitch, GA] = MA_Long(AtSS,Tau_Turb,G_Simple,Plano_Long,Kp,Kd,Plotar)
...
%Criado o modelo da Turbina no espaço de estados
...
for Alt=1:i_max
    ...
    for Vel=1:j_max
        ...
        GpitchSS = ss(Plano_Long(Alt,Vel).A(1:4,1:4), Plano_Long(Alt,Vel).B(1:4,:), eye(5),zeros(5,2));
        aux = append(Turb,AtSS);
        aux = series(aux,GpitchSS);
        K = [Kd.pitch(Alt,Vel) Kp.pitch(Alt,Vel)];       
        
        
        %Fechando a malha na Turbina
        GaAux = ss(aux.A-aux.B(:,1)*Kturb,aux.B(:,2),[0 0 1 zeros(1,4); 0 0 0 1 zeros(1,3)]);
        GA(Alt,Vel).PitchComplete = ss(GaAux.A,GaAux.B,K*aux.C,0); %Completa
